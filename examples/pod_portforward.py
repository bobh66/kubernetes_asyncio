...
from kubernetes_asyncio.stream import portforward
...
async def portforward_commands(api_instance):
    ...
    reader, writer = await asyncio.open_connection(sock=http)
    loop_ = asyncio.get_event_loop()
    loop_.call_soon(writer.write, b'GET / HTTP/1.1\r\n')
    loop_.call_soon(writer.write, b'Host: 127.0.0.1\r\n')
    loop_.call_soon(writer.write, b'Connection: close\r\n')
    loop_.call_soon(writer.write, b'Accept: */*\r\n')
    loop_.call_soon(writer.write, b'\r\n')
    response = b''
    while True:
    ...
    async def kubernetes_create_connection(address, *args, **kwargs):
        dns_name = address[0]
        ...
        if len(dns_name) == 4:
            if dns_name[1] in ('svc', 'service'):
                service = await api_instance.read_namespaced_service(name, namespace)
                ...
                raise RuntimeError(
                    ...
                    dns_name[1])
        pf_ = await portforward(api_instance.connect_get_namespaced_pod_portforward,
                                name, namespace, ports=str(port))
        return pf_.socket(port)
    socket.create_connection = kubernetes_create_connection

    # Access the nginx http server using the
    # "<pod-name>.pod.<namespace>.kubernetes" dns name.
    response = urllib_request.urlopen('http://%s.pod.default.kubernetes' % name)
...
async def main():
    ...
    await portforward_commands(core_v1)
